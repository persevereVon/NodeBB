...
function topicRoutes(app, middleware, controllers) {
    setupPageRoute(app, '/topic/:topic_id/:slug/:post_index?', middleware, [ 
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.topics.get);
    setupPageRoute(app, '/topic/:topic_id/:slug?', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.topics.get);
}

function postRoutes(app, middleware, controllers) {
    setupPageRoute(app, '/post/:pid', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.posts.redirectToPost);
}

function tagRoutes(app, middleware, controllers) {
    setupPageRoute(app, '/tags/:tag', middleware, [
    middleware.privateTagListing,
    ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.tags.getTag);
    setupPageRoute(app, '/tags', middleware, [
    middleware.privateTagListing,
    ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
...
function categoryRoutes(app, middleware, controllers) {
    setupPageRoute(app, '/categories', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.categories.list);
    setupPageRoute(app, '/popular', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.popular.get);
    setupPageRoute(app, '/recent', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.recent.get);
    setupPageRoute(app, '/top', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.top.get);
    setupPageRoute(app, '/unread', middleware, [
        middleware.authenticate,
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.unread.get);

    setupPageRoute(app, '/category/:category_id/:slug/:topic_index', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.category.get);
    setupPageRoute(app, '/category/:category_id/:slug?', middleware, [
        ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1'),
    ], controllers.category.get);
}

function userRoutes(app, middleware, controllers) {
    var middlewares = [middleware.checkGlobalPrivacySettings, ensureLoggedIn.ensureLoggedIn(nconf.get('relative_path') + '/login?local=1')];
